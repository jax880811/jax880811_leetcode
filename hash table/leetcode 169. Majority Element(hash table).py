...
"""
        LeetCode 169: 多数元素 (Majority Element) (Python 實現 - 優化哈希表計數)
        ...
        3. 解題步驟 (優化哈希表計數)：
           - 創建一個空的哈希表（Python 中的字典 `counts`）用於存儲每個數字及其出現的次數。
           - 獲取輸入陣列 `nums` 的長度 `n`。
           - 遍歷輸入陣列 `nums`。
           - 對於陣列中的每個數字 `num`：
             - 更新該數字在哈希表 `counts` 中的計數。使用 `counts[num] = counts.get(num, 0) + 1`，如果數字不存在則初始化為 1，否則加 1。
        ...
        重點筆記：
        1. **哈希表 (Hash Map) 的應用**:
           - 使用哈希表可以高效地統計每個元素的出現次數，平均情況下插入、查找和更新操作的時間複雜度都是 O(1)。在 Python 中，字典 (`dict`) 就是哈希表的實現。
        ...
        - `majorityElement(self, nums: List[int]) -> int`: 尋找眾數的核心方法 (使用優化哈希表計數)。
        - `counts = {}`: 初始化一個空的哈希表用於存儲元素計數。
        - `n = len(nums)`: 獲取陣列長度。
        - `for num in nums`: 遍歷輸入陣列。
        - `counts[num] = counts.get(num, 0) + 1`: 更新哈希表中數字的計數。使用 `get` 方法可以安全地處理鍵不存在的情況。
