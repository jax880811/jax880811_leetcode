...
/*
LeetCode 94: 二元樹的中序遍歷 (Binary Tree Inorder Traversal) (Node.js 實現 - 遞迴解法)

題目翻譯：
給定一個二元樹的根節點 `root`，返回其節點值的**中序**遍歷。

題目需求：
1. 實現二元樹的中序遍歷。
2. 返回遍歷結果，通常以陣列的形式呈現。
3. 中序遍歷的順序是：先遍歷左子樹，然後訪問根節點，最後遍歷右子樹。

解題思路與拆解：
1. 問題分析：
   - 需要按照特定的順序（左、根、右）訪問二元樹的所有節點。
   - 遞迴方法非常適合描述這種具有明確順序的遍歷方式.

2. 解題方法選擇：
   - 方法一：遞迴 (Recursive) (本解法採用) - 時間 O(N)，空間 O(H) (H 是樹的高度，最壞情況下為 O(N))

3. 解題步驟 (遞迴方法)：
   - 在 `Solution` 類別中定義一個名為 `inorderTraversal` 的方法，它接收二元樹的根節點 `root` 作為輸入。
   - 在 `inorderTraversal` 方法中，初始化一個空陣列 `result`，用於儲存中序遍歷的結果。
   ...
   - 在 `traverse` 函數中：
     ...
     - **步驟 2：訪問根節點。** 將當前節點的值 (`node.val`) 加入到 `result` 陣列中。
     - **步驟 3：遞迴遍歷右子樹。** 調用 `traverse(node.right)` 來遍歷當前節點的右子樹。
   - 在 `inorderTraversal` 方法中，調用 `traverse(root)` 開始從根節點進行遞迴遍歷。
   - 最後，返回儲存了中序遍歷結果的 `result` 陣列。

重點筆記：
1. **中序遍歷順序**: 左 -> 根 -> 右。
2. **遞迴的核心思想**: 將問題分解為更小的子問題，直到達到基本情況（節點為空）。
3. **遞迴調用的順序**: 先遞迴處理左子樹，然後處理當前節點，最後處理右子樹，完美符合中序遍歷的定義。
...
- `inorderTraversal(root)`: 實現二元樹中序遍歷的核心方法。
...
- `console.log(solution.inorderTraversal(tree_root));`: 調用方法並打印結果。
*/